...
def _label_to_int(x: Any) -> int | None:
    """Accept 0/1/2/3 or strings {'bending','idle','picking','pushing'} → 0/1/2/3. Return None if unknown."""
    if pd.isna(x):
        return None
    try:
        v = int(x)
        ...
        s = str(x).strip().lower()
        m = {"bending": 0, "idle": 1, "picking": 2, "pushing": 3}
        return m.get(s)
...
def train_xgb_if_available(train_csv: Path, out_model: Path) -> float | None:
    ...
    print("[train] XGBoost Confusion matrix:\n", confusion_matrix(y_test, y_pred))
    print("[train] XGBoost Report:\n", classification_report(y_test, y_pred, digits=3))
...
def _scan_existing_versions(models_dir: Path, activity: str, alg: str) -> int:
    ...
    if not models_dir.exists():
        return 0
    pat = re.compile(
        rf"^{re.escape(alg)}_{re.escape(activity)}_v(\d+)\.(pkl|joblib)$", re.IGNORECASE
    )
    max_n = 0
...
def _next_versioned_name(
    models_dir: Path, activity: str, alg: str, ext: str = "pkl"
) -> Path:
    """
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train activity-specific model from wide CSVs"
    )
    p.add_argument("activity", help="Activity name (e.g. pickup, shelf, spray)")
    p.add_argument(
        "--merged-root",
        default="merged",
        help="Root folder containing per-activity subfolders",
    )
    p.add_argument(
        "--models-root",
        default="models",
        help="Root folder to write per-activity models",
    )
    p.add_argument(
        "--fps",
        type=int,
        default=FRAMES_PER_SECOND_DEFAULT,
        help="Rows per 1-second window",
    )
...
def main() -> None:
    ...
    print(
        f"[config] auto-versioning : {'ON' if auto_versioning else 'OFF (fixed names)'}"
    )
