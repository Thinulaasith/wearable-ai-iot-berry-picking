...
# ---------------------------------------------------
# Hierarchical architecture with Other / rejection
# ---------------------------------------------------


def _predict_hierarchical_with_other(model1, model2, X):
    """
    Run the complete hierarchical with other pipeline and return final 5-class predictions.
    """
    stage1_pred = model1.predict(X)
    stage1_pred = pd.Series(stage1_pred, index=X.index)
    ...
    if picking_mask.any():
        ...
        good_indices = stage2_pred.index[stage2_pred == S2_GOOD]
        bad_indices = stage2_pred.index[stage2_pred == S2_BAD]
        other_indices = stage2_pred.index[stage2_pred == OTHER]

        final_pred.loc[good_indices] = GOOD_PICKING
        final_pred.loc[bad_indices] = BAD_PICKING

        if len(other_indices) > 0:
            probabilities = model1.predict_proba(X.loc[other_indices])

            # Exclude Picking and choose the strongest remaining class.
            non_picking = model1.classes_ != S1_PICKING
            fallback_classes = model1.classes_[non_picking]

            fallback_predictions = fallback_classes[
                probabilities[:, non_picking].argmax(axis=1)
            ]

            # Convert Stage 1 labels into final activity labels.
            stage1_to_final = {
                S1_BENDING: BENDING,
                S1_IDLE: IDLE,
                S1_PUSHING: PUSHING,
            ...
            final_pred.loc[other_indices] = pd.Series(
...
def train_hierarchical_other(
    train_csv: Path,
    study_dir: Path,
    model: ModelChoice = DEFAULT_MODEL_CHOICE,
) -> Dict[str, Dict[str, Any]]:
    """
    Train the hierarchical architecture with Other.

    Stage 1:
        Bending / Idle / Picking / Pushing

    Stage 2:
        Good Picking / Bad Picking / Other
    ...
    # ---------------------------------------------------
    # Create Stage 2 targets
    # ---------------------------------------------------

    y2_train = y_train.map(FINAL_TO_STAGE2_WITH_OTHER).astype(int)
    y2_test = y_test.map(FINAL_TO_STAGE2_WITH_OTHER).astype(int)

    results: Dict[str, Dict[str, Any]] = {}
    hierarchical_dir = study_dir / "hierarchical_with_other"
    results_dir = study_dir / "results"
    ...
    if model in ("both", "rf"):
        ...
        rf_stage2 = RandomForestClassifier(**RF_PARAMS)
        rf_stage2.fit(X_train, y2_train)
        ...
        stage1_pred = rf_stage1.predict(X_test)
        rf_stage1_results = evaluate_predictions(
            y1_test,
            stage1_pred,
            "stage1",
        )

        stage2_pred = rf_stage2.predict(X_test)
        rf_stage2_results = evaluate_predictions(
            y2_test,
            stage2_pred,
            "stage2_with_other",
        )
        ...
        final_pred = _predict_hierarchical_with_other(
            rf_stage1,
            rf_stage2,
            X_test,
        )
        ...
        rf_evaluation = {
            "combined": combined_result,
            "stage1": rf_stage1_results,
            "stage2": rf_stage2_results,
        }

        save_json(
            rf_evaluation,
            results_dir / "hierarchical_other_rf_results.json",
        )

        save_hierarchical_report(
            rf_evaluation,
            results_dir / "hierarchical_other_rf_report.md",
    ...
    if model in ("both", "xgb"):
        ...
        xgb_stage2.fit(X_train, y2_train)
        ...
        xgb_stage1_results = evaluate_predictions(
            y1_test,
            stage1_pred,
            "stage1",
        )

        stage2_pred = xgb_stage2.predict(X_test)

        xgb_stage2_results = evaluate_predictions(
            y2_test,
            stage2_pred,
            "stage2_with_other",
        )
        ...
        xgb_evaluation = {
            "combined": combined_result,
            "stage1": xgb_stage1_results,
            "stage2": xgb_stage2_results,
        }

        save_json(
            xgb_evaluation,
            results_dir / "hierarchical_with_other_xgb_results.json",
        )

        save_hierarchical_report(
            xgb_evaluation,
            results_dir / "hierarchical_with_other_xgb_report.md",
